...
def create_query_db(source_folder, dest_folder):
    ...
    for category in os.listdir(source_folder):
        ...
        models = [model for model in os.listdir(category_path) if model.endswith('.obj')]
...
def compute_distances(query_desc, db_desc):
    ...
    for doc in db_desc:
        dist_metrics = {
            ...
            'zernike_distance': compute_distance_zernike(query_desc['zernike'], np.array(doc['zernike']), metric='l2'),
            'fourier_distance': compute_distance_fourier(query_desc['fourier'], np.array(doc['fourier']), metric='l2')
        }
...
def calculate_similarities(distances, weights=None):
    ...
    for idx, dist_metrics in enumerate(distances):
        ...
        dist_metrics['similarity'] = 1 - (weights['zernike'] * norm_zernike[idx] + 
...
def plot_images(folder, rand_model, top_images, similarities, n=15):
    rows = 4
    cols = (n + 1) // rows + ((n + 1) % rows > 0)
    _, axes = plt.subplots(rows, cols, figsize=(10, 10))
    category = None
    for root, dirs, files in os.walk(folder):
        if rand_model.replace('.obj','.jpg') in files:
            category = root.replace('Thumbnails\\', '')
            break
    model1 = cv2.cvtColor(cv2.imread(os.path.join(folder, category, rand_model.replace('.obj','.jpg'))), cv2.COLOR_BGR2RGB)
    axes[0, 0].imshow(model1)
    axes[0, 0].set_title(f"Source:\n{rand_model}")
    axes[0, 0].axis("off")
    for i, (model_name, sim) in enumerate(top_images):
        row = (i + 1) // cols
        col = (i + 1) % cols
        model_path = os.path.join(folder, similarities[i]['category'], model_name+'.jpg')
        model2 = cv2.cvtColor(cv2.imread(model_path), cv2.COLOR_BGR2RGB)
        axes[row, col].imshow(model2, cmap='gray')
        axes[row, col].set_title(f"{similarities[i]['category']} - {model_name}\nSimilarity: {sim:.5f}")
...
def RetrieveModels(folder, query_desc, rand_model, n=15):
    if query_desc is None:
        ...
        plot_images(folder, rand_model, top_images, similarities, n)
