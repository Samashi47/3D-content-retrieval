...
def export_obj(filename, vertices, faces, normals, face_normals):
    ...
    with open(out_filename, "w") as f:
        ...
        for face, fn in zip(faces, face_normals):
            f.write(
                f"f {face[0]+1}//{fn[0]} {face[1]+1}//{fn[1]} {face[2]+1}//{fn[2]}\n"
            )
...
def main():
    ...
    export_obj(
        outfile, ec.new_vertices, ec.new_faces, ec.new_normals, ec.new_face_normals
    )

    print(f"Original mesh: {len(mesh.V)} vertices, {len(mesh.F)} faces")
    print(
        f"Simplified mesh: {len(ec.new_vertices)} vertices, {len(ec.new_faces)} faces"
    )
